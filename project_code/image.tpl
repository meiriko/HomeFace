		...
		<script type="text/javascript">
			...
			window.onload = function() {
				...
				raster.onLoad = function(){
					this.position = new paper.Point(this.width/2, this.height/2);
			...
			function rect(x,y,w,h,config){
				...
				var val = new paper.Path.Rectangle({{.DX}} + x * {{.SX}},{{.DY}} + y * {{.SY}},w * {{.SX}}, h * {{.SY}});
			...
			function path(points, config){
				...
				val.moveTo( new paper.Point( {{.DX}} + points[0][0] * {{.SX}},{{.DY}} + points[0][1] * {{.SY}} ) );
				for( var i = 1 ; i < points.length ; i++ ){
					val.lineTo( new paper.Point( {{.DX}} + points[i][0] * {{.SX}},{{.DY}} + points[i][1] * {{.SY}} ) );
				}
				return val;
			}
			...
			$.getJSON('configTitles', function(data) {
				...
				$select.on('change', function(){
					if( this.value && this.value.length ){
						...
						$.get('config',{key:this.value}, function(data){
							drawShapes(JSON.parse(JSON.parse(data).Config), mainGroup);
						});
					}
				});
